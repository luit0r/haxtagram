...
banner() {
printf "\e[1;77m\e[41m  PR0JECT HAXTAGRAM v1.1  \e[0m\n" 
printf "\n"
}
...
printf "\e[1;91mThis Username is not available! try again!\e[0m\n"
sleep 1
start
else
...
checkroot() {
if [[ "$(id -u)" -ne 0 ]]; then
    printf "\e[1;77mPLEASE RUN THIS SCRIPT AS SUPERUSER\n\e[0m"
    exit 1
fi
}
...
command -v tor > /dev/null 2>&1 || { echo >&2 "Cannot find TOR run ./archin.sh or ./debin.sh to install dependencies Good Bye! User."; exit 1; }
command -v curl > /dev/null 2>&1 || { echo >&2 "Cannot find cURL run ./archin.sh or ./debin.sh to install dependencies. Good Bye! User."; exit 1; }
command -v openssl > /dev/null 2>&1 || { echo >&2 "Cannot find openSSL run ./archin.sh or ./debin.sh to install dependencies Good Bye! User."; exit 1; }

command -v awk > /dev/null 2>&1 || { echo >&2 "Need awk , but it's not installed. Good Bye! User."; exit 1; }
command -v sed > /dev/null 2>&1 || { echo >&2 "Need sed , but it's not installed. Good Bye! User."; exit 1; }
command -v cat > /dev/null 2>&1 || { echo >&2 "Need cat , but it's not installed. Good Bye! User."; exit 1; }
command -v tr > /dev/null 2>&1 || { echo >&2 "Need tr , but it's not installed. Good Bye! User."; exit 1; }
command -v wc > /dev/null 2>&1 || { echo >&2 "Need wc , but it's not installed. Good Bye! User."; exit 1; }
command -v cut > /dev/null 2>&1 || { echo >&2 "Need cut , but it's not installed. Good Bye! User."; exit 1; }
command -v uniq > /dev/null 2>&1 || { echo >&2 "Need uniq , but it's not installed. Good Bye! User."; exit 1; }
...
printf "\n"
printf "\e[1;77m[*] Starting Bruteforcing\e[0m\n"
printf "\e[1;91m [*] Press Crtl + C to Stop or Save this session\e[0m\n"
sleep 2
}
...
if [[ -n "$threads" ]]; then
printf "\n"
printf "\e[1;91m [*] Waiting threads to shut down.\n\e[0m"
...
rm -rf nottested.lst
printf "\e[1;91m [!] Use this file as a preffered wordlist!\e[0m\n"
fi
default_session="Y"
...
printf "\e[1;92mUse ./haxter.sh --resume\n"
else
exit 1
fi
else
